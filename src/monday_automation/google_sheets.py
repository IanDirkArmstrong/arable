"""Google Sheets integration for Monday automation"""
...
class GoogleSheetsClient:
    ...
    def read_data(self) -> Tuple[List[Dict], List[Dict]]:
        """
        Read both projects and milestones data with RSi validation
        ...
        # RSi milestone validation (if validator is available)
        try:
            from .rsi_validation import RSiMilestoneValidator

            # Get config from somewhere - we'll need to pass this in
            # For now, just log that we found milestones
            milestone_types = set(
            ...
            for milestone_type in sorted(milestone_types):
                self.logger.info(f"   - {milestone_type}")

        except ImportError:
            self.logger.debug("RSi validation not available")
